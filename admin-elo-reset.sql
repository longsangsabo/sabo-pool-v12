...
BEGIN
  ...
  UPDATE player_rankings 
  SET elo_points = CASE 
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'E+' THEN 2800
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'E' THEN 2600
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'F+' THEN 2400
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'F' THEN 2200
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'G+' THEN 2000
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'G' THEN 1800
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'H+' THEN 1600
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'H' THEN 1400
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'I+' THEN 1200
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'I' THEN 1000
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'K+' THEN 800
    WHEN (SELECT verified_rank FROM profiles WHERE user_id = player_rankings.player_id) = 'K' THEN 600
    ELSE 1000 -- Default cho unranked/beginner
  END,
  updated_at = NOW();
