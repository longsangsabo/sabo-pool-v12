     ...
     <div className='hidden lg:flex lg:items-center lg:space-x-8'>
      {navigationItems.map(item => (
       <Link
        ...
        className={`px-3 py-2 text-body-small-medium transition-colors ${
         item.current
      ...
      {user && (
       <Link
        to='/inbox'
        className={`relative px-3 py-2 text-body-small-medium transition-colors ${
       ...
       >
        <div className='relative'>
         ...
         {unreadCount > 0 && (
          <Badge
           className='absolute -top-2 -right-2 h-4 w-4 p-0 text-caption bg-accent-red hover:bg-accent-red flex items-center justify-center'
           variant='destructive'
          >
        ...
        <DropdownMenuContent className='w-56' align='end' forceMount>
         <DropdownMenuLabel className='font-normal'>
          <div className='flex flex-col space-y-1'>
           <p className='text-body-small-medium leading-none'>
      ...
      <div className='px-2 pt-2 pb-3 space-y-1'>
       ...
       <div className='px-3 py-2'>
        <div className='flex items-center justify-between'>
         <span className='text-body-small-medium text-muted-foreground'>
          Giao diện
         </span>
         <ThemeToggle />
        </div>
       ...
       {navigationItems.map(item => (
        <Link
         key={item.name}
         to={item.href}
         className={`block px-3 py-2 text-body-medium rounded-md transition-colors ${
          item.current
       ...
       {user && (
        <Link
         to='/inbox'
         className={`flex items-center justify-between px-3 py-2 text-body-medium rounded-md transition-colors ${
          location.pathname === '/inbox'
           ? 'text-primary bg-primary/10'
           : 'text-muted-foreground hover:text-primary hover:bg-muted/50'
         }`}
        ...
        >
         ...
         {unreadCount > 0 && (
          <Badge
           className='h-5 w-5 p-0 text-caption bg-accent-red hover:bg-accent-red flex items-center justify-center'
           variant='destructive'
          >
           {unreadCount > 99 ? '99+' : unreadCount}
          </Badge>
       ...
       {isAdmin && (
        <Link
         to='/admin'
         className={`flex items-center px-3 py-2 text-body-medium rounded-md transition-colors ${
          location.pathname === '/admin'
       ...
       {!user && (
        <div className='pt-4 border-t border-border'>
         <Link
          to='/login'
          className='block px-3 py-2 text-body-medium text-muted-foreground hover:text-foreground hover:bg-muted/50 rounded-md transition-colors'
          onClick={() => setIsMenuOpen(false)}
         >
          Đăng nhập
         </Link>
         <Link
          to='/register'
          className='block px-3 py-2 text-body-medium bg-gradient-to-r from-primary to-primary-glow text-primary-foreground rounded-md mt-2 transition-colors'
          onClick={() => setIsMenuOpen(false)}
         >
          Đăng ký
         </Link>
