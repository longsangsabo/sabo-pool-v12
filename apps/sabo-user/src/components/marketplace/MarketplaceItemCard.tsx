...
const MarketplaceItemCard = ({
 ...
 const getConditionVariant = (condition: string | undefined) => {
  switch (condition) {
   case 'new':
    return 'default';
   case 'used':
 ...
 return (
  <Card className='group hover:shadow-lg transition-shadow cursor-pointer'>
   <CardContent className='p-0'>
    {/* Product Image */}
    <div className='relative aspect-square bg-neutral-100 rounded-t-lg overflow-hidden'>
    ...
    <div className='p-4 space-y-3' onClick={() => onItemClick(item)}>
     {/* Title and Price */}
     <div>
      <h3 className='font-semibold text-body-large line-clamp-2 group-hover:text-primary-600 transition-colors'>
       {item.title}
      </h3>
      <div className='flex items-center justify-between mt-1'>
       <span className='text-title font-bold text-success-600'>
        {formatPrice(item.price)}
       </span>
       {item.original_price && item.original_price > item.price && (
        <span className='text-body-small text-neutral-500 line-through'>
     ...
     {/* Description */}
     {item.description && (
      <p className='text-neutral-600 text-body-small line-clamp-2'>
       {item.description}
      </p>
     )}
     ...
     {/* Location and Stats */}
     <div className='flex items-center justify-between text-body-small text-neutral-500'>
     ...
     {item.seller && (
      <div className='flex items-center justify-between pt-2 border-t'>
       ...
       {item.seller.avg_response_time && (
        <div className='flex items-center gap-1 text-caption-neutral'>
