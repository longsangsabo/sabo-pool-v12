...
final tournamentsProvider = FutureProvider<List<Map<String, dynamic>>>((ref) async {
  return await DataService.getTournaments();
});

final userTournamentsProvider = FutureProvider<List<Map<String, dynamic>>>((ref) async {
...
// Club providers
final clubsProvider = FutureProvider<List<Map<String, dynamic>>>((ref) async {
  return await DataService.getClubs();
});

final userClubsProvider = FutureProvider<List<Map<String, dynamic>>>((ref) async {
  final authState = ref.watch(realAuthStateProvider);
  if (!authState.isAuthenticated || authState.user == null) {
    return [];
  }
...
class UpdateProfileNotifier extends StateNotifier<AsyncValue<void>> {
  ...
  Future<void> updateProfile(Map<String, dynamic> updates) async {
    ...
    try {
      ...
      ref.invalidate(realAuthStateProvider);
...
// Current user data provider for profile screen
final currentUserDataProvider = FutureProvider<Map<String, dynamic>>((ref) async {
  ...
  final userData = await DataService.getUserProfile(authState.user!.id);
  final userStats = await DataService.getUserStatistics(authState.user!.id);
