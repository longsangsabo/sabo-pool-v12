...
class RealAuthNotifier extends StateNotifier<AppAuth.AuthState> {
  ...
  Future<void> _checkAuthStatus() async {
    try {
      ...
      state = AuthStateError(message: 'Kiểm tra auth thất bại: $e');
    }
  }
  ...
  Future<void> _updateAuthenticatedState(User supabaseUser) async {
    try {
      ...
      state = AuthStateAuthenticated(user: user, token: supabaseUser.id);
    } catch (e) {
  ...
  Future<void> _createUserRecord(User supabaseUser) async {
    try {
      ...
      state = AuthStateError(message: 'Tạo user record thất bại: $e');
    }
  }
  ...
  Future<void> login({
    ...
    try {
      ...
      if (response.user != null) {
        ...
        state = const AuthStateError(message: 'Đăng nhập thất bại');
      }
    } catch (e) {
      state = AuthStateError(message: 'Đăng nhập thất bại: ${e.toString()}');
    }
  }
  ...
  Future<void> register({
    ...
    try {
      ...
      if (response.user != null) {
        // _updateAuthenticatedState sẽ được gọi tự động qua auth state listener
        state = const AuthStateOtpVerified(); // Cần xác thực email
      } else {
        state = const AuthStateError(message: 'Đăng ký thất bại');
      }
    } catch (e) {
      state = AuthStateError(message: 'Đăng ký thất bại: ${e.toString()}');
    }
  }
  ...
  Future<void> verifyOTP({
    ...
    try {
      ...
      state = AuthStateError(message: 'Xác thực OTP thất bại: ${e.toString()}');
    }
  }
  ...
  Future<void> sendPasswordReset({
    ...
    try {
      ...
      state = AuthStateError(message: 'Gửi reset password thất bại: ${e.toString()}');
    }
  }
  ...
  Future<void> logout() async {
    try {
      ...
      state = AuthStateError(message: 'Đăng xuất thất bại: ${e.toString()}');
    }
  }
