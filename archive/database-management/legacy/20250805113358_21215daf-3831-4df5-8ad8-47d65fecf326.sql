-- Tạo function sabo_tournament_coordinator để xử lý tiến triển SABO tournament
...
BEGIN
  ...
  IF NEW.status = 'completed' AND OLD.status != 'completed' AND NEW.winner_id IS NOT NULL THEN
    ...
    -- Update match status to scheduled cho các round tiếp theo
...
-- Tạo trigger cho SABO tournament progression
