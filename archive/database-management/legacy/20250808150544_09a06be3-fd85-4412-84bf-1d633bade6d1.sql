-- Create optimized indexes for automation queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tournament_matches_automation 
  ON tournament_matches (tournament_id, status, winner_id, round_number);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tournament_automation_log_perf 
  ON tournament_automation_log (tournament_id, automation_type, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_automation_performance_log_type_time 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_player_rankings_leaderboard 
...
AS $$
BEGIN
  REFRESH MATERIALIZED VIEW CONCURRENTLY public.mv_leaderboard_stats;
END;
$$;