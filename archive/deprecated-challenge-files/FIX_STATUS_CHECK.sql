-- FIX STATUS CHECK - ACCEPT BOTH 'pending' AND 'open'
-- ===================================================
...
BEGIN
    ...
    BEGIN
        ...
        SELECT 
            ...
            c.challenger_id,
            c.opponent_id,
            c.bet_points as spa_amount,
            c.status,
            c.club_id,
            cr.verified_rank as challenger_rank,
            pr.verified_rank as current_user_rank
        INTO challenge_record
        FROM challenges c
        LEFT JOIN profiles cr ON cr.user_id = c.challenger_id
        LEFT JOIN profiles pr ON pr.user_id = p_user_id
        ...
        -- ✅ FIXED: Accept both 'pending' and 'open' status
        IF challenge_record.status NOT IN ('open', 'pending') THEN
            RETURN json_build_object(
                'success', false,
        ...
        FROM profiles WHERE user_id = challenge_record.challenger_id;
        ...
        SET spa_balance = spa_balance - required_spa,
            updated_at = NOW()
        WHERE user_id = challenge_record.challenger_id;
...
-- Test function
SELECT 'accept_open_challenge_v3 FIXED for pending status!' as status;
