...
app.listen(PORT, () => {
  ...
  console.log(
    `   - Create payment: POST http://localhost:${PORT}/api/payments/create-vnpay`
  );
  console.log(
    `   - Payment return: GET http://localhost:${PORT}/api/webhooks/vnpay-return`
  );
  console.log(
    `   - Payment IPN: GET http://localhost:${PORT}/api/webhooks/vnpay-ipn`
  );
  console.log(
    `   - Payment status: GET http://localhost:${PORT}/api/payments/vnpay-status/:orderId`
  );
  console.log(
    `   - Demo: GET http://localhost:${PORT}/api/demo/create-payment`
  );
  console.log('');
  console.log('🔧 VNPAY Test Configuration:');
  console.log(`   - TMN Code: ${process.env.VNP_TMN_CODE || 'T53WMA78'}`);
  console.log(
    `   - Payment URL: ${process.env.VNP_PAYMENT_URL || 'https://sandbox.vnpayment.vn/paymentv2/vpcpay.html'}`
  );
  console.log('');
