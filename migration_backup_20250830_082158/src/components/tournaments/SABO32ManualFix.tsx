            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                {analysis.groupAFinals.map((final: any) => (
                  <div key={final.id} className="text-xs p-2 bg-white dark:bg-gray-800 rounded border">
                    <Badge variant={final.completed ? "default" : "secondary"} className="mr-2">
                      {final.sabo_match_id}
                    </Badge>
                    {final.completed ? (
                      <span className={final.winner_advanced ? "text-green-600" : "text-orange-600"}>
            ...
            <div>
              ...
              <div className="space-y-1">
                {analysis.crossSemis.map((semi: any) => (
                  <div key={semi.id} className="text-xs p-2 bg-white dark:bg-gray-800 rounded border">
                    <Badge variant="outline" className="mr-2">{semi.sabo_match_id}</Badge>
                    <span className={semi.player1_id ? "text-green-600" : "text-gray-500"}>
                      Player 1: {semi.player1_id ? "Set" : "TBD"}
                    </span>
                    <span className="mx-2">|</span>
                    <span className={semi.player2_id ? "text-green-600" : "text-gray-500"}>
                ...
                {analysis.crossFinal.map((final: any) => (
                  <div key={final.id} className="text-xs p-2 bg-white dark:bg-gray-800 rounded border">
                    ...
                    <span className={final.player1_id ? "text-green-600" : "text-gray-500"}>
                      Player 1: {final.player1_id ? "Set" : "TBD"}
                    </span>
                    <span className="mx-2">|</span>
                    <span className={final.player2_id ? "text-green-600" : "text-gray-500"}>
