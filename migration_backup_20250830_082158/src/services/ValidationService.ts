...
export class ValidationService {
  ...
  private static validateTournamentName(name: string): string | undefined {
  ...
  private static validateVenueAddress(address: string): string | undefined {
    if (!address?.trim()) return 'Địa điểm tổ chức là bắt buộc';
    if (address.trim().length < 5) return 'Địa điểm phải có ít nhất 5 ký tự';
    return null;
  }

  private static validateMaxParticipants(count: number): string | undefined {
  ...
  private static validateEntryFee(fee: number): string | undefined {
    if (fee < 0) return 'Phí đăng ký không được âm';
    return null;
  }

  private static validateTournamentStart(startTime: string): string | undefined {
  ...
  private static validateRegistrationStart(regStart: string): string | undefined {
  ...
  private static validateContactInfo(contactInfo: string): string | undefined {
