-- Migration: Fix duplicate key constraint in accept_open_challenge
-- File: supabase/migrations/20250813000001_fix_duplicate_challenge_match.sql
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    ELSIF FOUND AND v_challenge.status != 'pending' THEN
      RETURN jsonb_build_object(
        'success', false,
  ...
  IF FOUND THEN
    ...
    ELSE
      -- Match exists but with different players - should not happen
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Thách đấu này đã có trận đấu được tạo với người khác'
      );
    END IF;
  END IF;
  ...
  -- Create match record with ON CONFLICT handling
  ...
  ON CONFLICT (challenge_id) DO UPDATE SET
    ...
    updated_at = NOW()
  WHERE matches.player1_id = v_challenge.challenger_id;
...
-- Add comment
COMMENT ON FUNCTION public.accept_open_challenge(UUID, UUID) IS 'Accept open challenge with SPA points validation and duplicate prevention';
