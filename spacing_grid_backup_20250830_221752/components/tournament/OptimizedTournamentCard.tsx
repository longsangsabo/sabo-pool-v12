...
const OptimizedTournamentCard: React.FC<OptimizedTournamentCardProps> = ({
  ...
  const getStatusInfo = () => {
    ...
    if (tournament.status === 'ongoing') {
      return { status: 'ongoing', text: 'Đang diễn ra', color: 'bg-success-500' };
    }

    if (tournament.status === 'cancelled') {
      return { status: 'cancelled', text: 'Đã hủy', color: 'bg-error-500' };
    }
    ...
    if (currentParticipants >= tournament.max_participants) {
      return { status: 'full', text: 'Đã đầy', color: 'bg-error-500' };
    }
    ...
    <Card
      className={`w-full hover:shadow-lg transition-shadow duration-200 overflow-hidden relative ${
        ...
        theme === 'dark'
          ...
          : 'border-neutral-300/80 shadow-lg shadow-gray-300/30 hover:shadow-gray-400/40'
      }`}
    >
      {/* Background Image */}
      <div
          ...
          <CardTitle
            className={`${isMobile ? 'text-base' : 'text-lg'} font-bold line-clamp-2 flex-1 mr-3 ${
              theme === 'dark'
                ...
                : 'text-neutral-900 drop-shadow-md'
            }`}
          >
            {tournament.name}
          </CardTitle>
          <Badge
            className={`${statusInfo.color} text-white text-caption px-1.5 py-0.5 rounded-md whitespace-nowrap font-medium flex-shrink-0 ${
              isMobile ? 'text-[10px]' : ''
            }`}
          >
            {statusInfo.text}
      ...
      >
        ...
        >
          ...
          <span
            className={`${
              theme === 'dark'
                ? 'text-gray-200 drop-shadow-md'
                : 'text-neutral-600 drop-shadow-sm'
            }`}
          >
          ...
          <span
            className={`text-caption px-2 py-1 rounded-full font-medium ${
              tournament.max_participants - currentParticipants > 0
                ? theme === 'dark'
                  ? 'bg-success-500/20 text-green-400 border border-green-500/40'
                  : 'bg-success-100 text-success-700'
                : theme === 'dark'
                  ? 'bg-error-500/20 text-red-400 border border-red-500/40'
        ...
        <div
          className={`flex items-center space-x-2 ${isMobile ? 'text-xs' : 'text-sm'} ${
            theme === 'dark' ? 'text-white drop-shadow-sm' : 'text-neutral-600'
          }`}
        >
        ...
        {showActions && (
          <div className={`flex space-x-2 ${isMobile ? 'pt-1' : 'pt-2'}`}>
            <Button
              ...
              className={`flex-1 ${isMobile ? 'text-caption h-8' : ''}`}
            >
              Chi tiết
            </Button>
            {((statusInfo.status === 'registration_open' &&
              ...
              <Button
                ...
                className={`flex-1 flex items-center gap-2 ${isMobile ? 'text-caption h-8' : ''} ${
                  isRegistered
                    ? 'bg-error-600 hover:bg-red-700 text-white'
