...
export const AchievementsCard: React.FC<AchievementsCardProps> = ({
  ...
  const generateAchievements = () => {
    ...
    if (stats.total_matches >= 10) {
      achievements.push({
        iconColor: theme === 'dark' ? 'text-blue-300' : 'text-primary-600',
        bg:
          theme === 'dark'
        ...
        icon: (
          <Medal
            className={`w-6 h-6 ${theme === 'dark' ? 'text-blue-300' : 'text-primary-600'}`}
          />
        ),
      });
    }
      ...
      <CardContent className='p-5 space-y-4'>
        {loading ? (
          ...
          <div className={`text-center py-8 ${
            ...
            <p className="text-body-small">Hãy thi đấu để mở khóa thành tích!</p>
          </div>
        ) : (
          achievements.map((a, i) => (
            ...
            >
              <div className='flex items-center gap-4 p-4'>
                ...
                >
                  <div className='text-body-small font-bold'>{a.points}</div>
                  <div className='text-caption opacity-80'>{a.secondary}</div>
                </div>
              </div>
            </div>
          ))
