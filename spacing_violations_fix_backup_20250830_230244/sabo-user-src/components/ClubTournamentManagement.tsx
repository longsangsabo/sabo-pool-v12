          ...
          {tournaments.length > 0 && (
            <div className='flex gap-2'>
              {Object.entries(
                ...
                <span
                  key={status}
                  className='text-caption bg-muted px-2 py-1 rounded'
                >
                  {status}: {count}
                </span>
              ))}
    ...
    if (!clubId) {
      return (
        <div className='text-center py-8'>
          <Typography variant="body" size="md">Không tìm thấy thông tin club. Vui lòng kiểm tra profile.</Typography>
        </div>
      );
    }
      ...
      <Card>
        ...
        <CardContent className='space-y-4'>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            <div>
              <Typography variant="heading" size="md">Thông tin cơ bản</Typography>
              <div className='space-y-2'>
                ...
                <div>
                  <span className='text-body-small text-muted-foreground'>Mô tả:</span>
                  <Typography variant="body" size="md">{tournament.description}</Typography>
                </div>
                <div>
                  <span className='text-body-small text-muted-foreground'>
                    Loại giải:
                  </span>
                  <Typography variant="body" size="md">{tournament.tournament_type}</Typography>
                </div>
                <div>
                  <span className='text-body-small text-muted-foreground'>
                    Trạng thái:
                  </span>
                  <Typography variant="body" size="md">{tournament.status}</Typography>
                </div>
              </div>
            </div>
            <div>
              <Typography variant="heading" size="md">Thời gian & Tham gia</Typography>
              <div className='space-y-2'>
                <div>
                  <span className='text-body-small text-muted-foreground'>
                    Bắt đầu:
                ...
                <div>
                  ...
                  <Typography variant="body" size="md">0/{tournament.max_participants}</Typography>
                </div>
                <div>
                  <span className='text-body-small text-muted-foreground'>
                    Phí tham gia:
                  </span>
                  <Typography variant="body" size="md">{tournament.entry_fee.toLocaleString('vi-VN')} VNĐ</Typography>
                </div>
              </div>
            </div>
          </div>
...
const TournamentParticipantsView = ({
  ...
  const getPaymentStatusBadge = (status: string) => {
    switch (status) {
      case 'completed':
        return (
          <Badge className='bg-success-100 text-success-800'>Đã thanh toán</Badge>
        );
      case 'pending':
        return (
      ...
      case 'failed':
        return (
          <Badge className='bg-error-100 text-error-800'>Thanh toán thất bại</Badge>
        );
      default:
        return (
          <Badge className='bg-neutral-100 text-neutral-800'>Chưa thanh toán</Badge>
        );
    }
  };
  ...
  const getRegistrationStatusBadge = (status: string) => {
    switch (status) {
      case 'confirmed':
        return <Badge className='bg-primary-100 text-primary-800'>Đã xác nhận</Badge>;
      case 'pending':
        return (
          <Badge className='bg-warning-100 text-warning-800'>Chờ xác nhận</Badge>
        );
      case 'cancelled':
        return <Badge className='bg-error-100 text-error-800'>Đã hủy</Badge>;
      default:
                  ...
                  <div className='flex items-center justify-between'>
                    <div className='flex-1'>
                      ...
                      <div className='text-body-small text-muted-foreground space-y-1'>
                        ...
                        {registration.notes && (
                          <Typography variant="body" size="md">💬 Ghi chú: {registration.notes}</Typography>
                        )}
                      </div>
                    </div>
      ...
      <div className='space-y-6'>
        ...
        {tournament.status === 'ongoing' && (
          <Card className='border-error-200 bg-error-50'>
            <CardHeader>
              <CardTitle className='flex items-center gap-2 text-error-800'>
            ...
            <CardContent>
              <div className='space-y-4'>
                <div className='bg-white rounded p-4 border'>
                  <Typography variant="heading" size="md">Quy trình đóng giải đấu:</Typography>
                  <ol className='text-body-small text-muted-foreground space-y-1 list-decimal list-inside'>
                ...
                <Button
                  ...
                  disabled={isGenerating || isValidating}
                  className='w-full bg-error-600 hover:bg-red-700'
                  size='lg'
                >
          ...
          <CardContent>
            <div className='space-y-4'>
              <div className='border rounded-lg p-4'>
                <Typography variant="heading" size="md">Thông tin hiện tại</Typography>
    ...
    <Card>
      ...
      <CardContent>
        <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mb-4'>
          <div>
            <p className='text-body-small-medium'>Loại giải</p>
            <p className='text-body-small text-muted-foreground'>
              {getTournamentTypeLabel(tournament.tournament_type)}
            </p>
          </div>
          <div>
            <p className='text-body-small-medium'>Người tham gia</p>
            <p className='text-body-small text-muted-foreground'>
              0/{tournament.max_participants}
            </p>
          </div>
          <div>
            <p className='text-body-small-medium'>Phí tham gia</p>
            <p className='text-body-small text-muted-foreground'>
              {tournament.entry_fee.toLocaleString('vi-VN')} VNĐ
            </p>
          </div>
          <div>
            <p className='text-body-small-medium'>Bắt đầu</p>
            <p className='text-body-small text-muted-foreground'>
