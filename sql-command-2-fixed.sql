-- Fixed version of claim_legacy_spa_points function
-- This removes references to player_rankings table which was causing the error
...
BEGIN
  ...
  BEGIN
    ...
    SET 
      spa_points = v_current_spa_points + v_legacy_entry.spa_points,
      updated_at = NOW()
    WHERE user_id = v_user_id;

    -- Try to create SPA transaction record (skip if table doesn't exist properly)
    BEGIN
    ...
    v_result := json_build_object(
      ...
      'new_total_spa_points', (v_current_spa_points + v_legacy_entry.spa_points)
    );
...
-- Add comment
COMMENT ON FUNCTION public.claim_legacy_spa_points(TEXT) IS 'Fixed direct code claim system for legacy SPA points - removed player_rankings dependency';
