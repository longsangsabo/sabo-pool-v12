...
const AuthToastContainer: React.FC<AuthToastContainerProps> = ({ toasts, onRemove }) => {
  const containerElement = document.getElementById('auth-toast-container') || document.body;
...
export const AuthToastProvider: React.FC<AuthToastProviderProps> = ({ children }) => {
  ...
  return (
    <AuthToastContext.Provider value={{ addToast, removeToast, clearAllToasts }}>
      {children}
...
export const useAuthToastHelpers = () => {
  ...
  return {
    ...
    showRegistrationSuccess: () => addToast({
      type: 'success',
    ...
    showEmailVerificationSent: (email: string) => addToast({
      type: 'info',
    ...
    showOtpSent: (phone: string) => addToast({
      type: 'info',
    ...
    showAuthError: (error: string) => addToast({
      type: 'error',
      title: 'Lỗi xác thực',
      message: error,
      duration: 6000
    ...
    showPasswordResetSent: (email: string) => addToast({
      type: 'success',
    ...
    showAccountLocked: () => addToast({
      type: 'warning',
      title: 'Tài khoản bị khóa',
      message: 'Tài khoản của bạn đã bị khóa tạm thời do đăng nhập sai nhiều lần.',
      duration: 8000,
      action: {
