  ...
  const WrappedComponent = (props: P) => {
    ...
    if (requiredRole) {
      ...
      if (userRole !== requiredRole) {
        return (
          <UnauthorizedPage requiredRole={requiredRole} userRole={userRole} />
        );
      }
    }
    ...
    <div className='max-w-md w-full space-y-8 text-center'>
      <div>
        ...
        <div className='mt-4 text-xs text-gray-500'>
          <p>
            Yêu cầu: <span className='font-semibold'>{requiredRole}</span>
          </p>
          <p>
            Của bạn: <span className='font-semibold'>{userRole}</span>
          </p>
        </div>
      </div>
...
export const withClubOwnerAuth = <P extends object>(
  Component: React.ComponentType<P>
