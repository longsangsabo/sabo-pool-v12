...
const EnhancedAvatar: React.FC<EnhancedAvatarProps> = ({
  ...
  const getRankConfig = (rank?: string, points?: number) => {
    ...
    if (points && points >= 2000) {
      return { color: 'bg-purple-500', icon: Crown, label: 'Huyền thoại' };
    } else if (points && points >= 1800) {
      return { color: 'bg-yellow-500', icon: Crown, label: 'Vua break' };
    } else if (points && points >= 1600) {
      return { color: 'bg-blue-500', icon: Star, label: 'Tay cơ số 1' };
    } else if (points && points >= 1400) {
      return { color: 'bg-green-500', icon: Zap, label: 'Thành thạo' };
    } else if (points && points >= 1200) {
      return { color: 'bg-orange-500', icon: Star, label: 'Triển vọng' };
    }
