...
import { Trophy, Users, Target, Star, Activity, UserPlus, Settings2, ShieldCheck, BarChart3, Calendar, TrendingUp } from 'lucide-react';
...
export const ClubOwnerDashboardMobile: React.FC<ClubOwnerDashboardMobileProps> = ({
  clubId,
  onNavigateMembers,
  ...
  const { data: activities = [] } = useClubActivities(clubId, { limit: 10 }, true);

  const stats = useMemo(() => [
    { label: 'Thành viên', value: club?.member_count ?? members.length, icon: Users, accent: 'text-emerald-500' },
    { label: 'Trận', value: club?.total_matches ?? 0, icon: Target, accent: 'text-blue-500' },
  ...
  return (
    <div className='min-h-screen pb-24 px-4 pt-4 space-y-6'>
      ...
      {/* Quick Owner Actions */}
      <div className='grid grid-cols-2 gap-3'>
        <Button variant='outline' className='justify-start gap-2 h-12 text-left' onClick={onInvite}>
          <UserPlus className='w-4 h-4 text-emerald-500 flex-shrink-0' />
          <span className='text-sm font-medium'>Mời thành viên</span>
        </Button>
        <Button variant='outline' className='justify-start gap-2 h-12 text-left' onClick={onSettings}>
          <Settings2 className='w-4 h-4 text-blue-500 flex-shrink-0' />
          <span className='text-sm font-medium'>Cài đặt CLB</span>
        </Button>
        <Button variant='outline' className='justify-start gap-2 h-12 text-left' onClick={onNavigateMembers}>
          <Users className='w-4 h-4 text-indigo-500 flex-shrink-0' />
          <span className='text-sm font-medium'>Quản lý thành viên</span>
        </Button>
        <Button variant='outline' className='justify-start gap-2 h-12 text-left' onClick={onNavigateActivities}>
      ...
      >
        <div className='space-y-2'>
          ...
          {activities.slice(0,5).map(a => (
            <div key={a.id} className='flex items-center gap-3 p-2 rounded-md border text-sm bg-muted/40 dark:bg-slate-800/40'>
              <Activity className='w-4 h-4 text-blue-500 flex-shrink-0' />
              <div className='flex-1 min-w-0'>
                <p className='truncate font-medium'>{a.content}</p>
                <p className='text-xs text-muted-foreground mt-0.5'>{new Date(a.created_at).toLocaleDateString('vi-VN')}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      >
        <p className='text-sm text-muted-foreground'>
          Thống kê chi tiết về hiệu suất hoạt động CLB sẽ được cập nhật trong phiên bản tới.
        </p>
      </MobileCard>
    </div>
  );
