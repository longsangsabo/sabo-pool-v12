  ...
  return (
    <Card className={compact ? 'h-full' : ''}>
      ...
      <CardContent className='pt-0'>
        {isLoading && <div className='text-xs text-muted-foreground'>Đang tải...</div>}
        {isError && <div className='text-xs text-red-500'>Lỗi: {(error as any)?.message}</div>}
        {!isLoading && !isError && (!data || data.length === 0) && (
          <div className='text-xs text-muted-foreground'>Chưa có thành tựu nào</div>
        )}
        <ul className='space-y-2 max-h-64 overflow-auto pr-1'>
          {data?.map((aw) => {
            ...
            return (
              <li key={aw.awarded_at + (aw.milestone_id || '')} className='flex items-start gap-2 text-xs border-b last:border-b-0 pb-1'>
