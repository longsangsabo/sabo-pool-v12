...
export const DesktopProfileHero: React.FC<DesktopProfileHeroProps> = ({
  ...
  return (
    <section aria-labelledby='profile-hero-heading' className='w-full'>
      <h1 id='profile-hero-heading' className='sr-only'>Hồ sơ người chơi</h1>
      <div
      ...
      >
        ...
        <div className='relative z-10 flex flex-col md:flex-row md:items-end gap-10'>
          <div className='flex justify-center md:justify-start'>
            <div className={`relative rounded-2xl p-[2px] ${theme==='dark' ? 'bg-gradient-to-br from-blue-500/60 via-indigo-500/40 to-transparent' : 'bg-gradient-to-br from-blue-300 via-blue-100 to-transparent'}`}>
              <div className={`rounded-[14px] ${theme==='dark' ? 'bg-slate-950/60' : 'bg-white'} backdrop-blur-sm`}> 
                {theme === 'dark' ? (
                  <DarkCardAvatar
                    userAvatar={profile?.avatar_url}
                    onAvatarChange={onAvatarUpload}
                    uploading={uploading}
                    nickname={profile?.display_name || profile?.full_name || 'USER'}
                    rank={profile?.verified_rank || 'K'}
                    elo={profile?.win_percentage ? Math.round(profile.win_percentage * 10) : 0}
          ...
          <div className='flex-1'>
            ...
            <div className='mt-5 flex flex-wrap gap-3'>
              ...
              <Button size='sm' variant='secondary' onClick={onNavigateClub} className='flex items-center gap-1'>
                <Building className='w-3.5 h-3.5' /> CLB
              </Button>
              <Button size='sm' variant='secondary' onClick={onNavigateRank} className='flex items-center gap-1'>
                <Shield className='w-3.5 h-3.5' /> Hạng
              </Button>
              <Button size='sm' variant='secondary' onClick={onNavigateCommunity} className='flex items-center gap-1'>
                <UsersIcon className='w-3.5 h-3.5' /> Cộng đồng
              </Button>
              <Button size='sm' onClick={onRequestRank}>Yêu cầu thay đổi hạng</Button>
            </div>
            ...
            <div className='mt-4 flex flex-wrap gap-2'>
              {profile?.verified_rank && (
                <span className={`text-[10px] font-semibold px-2 py-1 rounded-full border ${
                  theme==='dark' ? 'bg-blue-500/10 text-blue-300 border-blue-500/30' : 'bg-blue-50 text-blue-700 border-blue-200'
                }`}>RANK: {profile.verified_rank}</span>
              )}
              {profile?.role === 'admin' && (
                <span className={`text-[10px] font-semibold px-2 py-1 rounded-full flex items-center gap-1 border ${
                  theme==='dark' ? 'bg-red-500/10 text-red-300 border-red-500/30' : 'bg-red-50 text-red-700 border-red-200'
                }`}>ADMIN</span>
              )}
