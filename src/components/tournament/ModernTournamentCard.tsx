...
const ModernTournamentCard: React.FC<ModernTournamentCardProps> = ({
  ...
  const getStatusInfo = () => {
    ...
    const regEndDate = tournament.registration_end
  ...
  const registrationProgress =
    (currentParticipants / tournament.max_participants) * 100;

  // Premium tournament detection
  const isPremium =
    tournament.tier_level === 'premium' ||
    (tournament.entry_fee && tournament.entry_fee > 100000);
  const isHot =
    statusInfo.status === 'registration_open' && availableSlots <= 5;
  ...
  const getHeroBackground = () => {
    ...
    return {
      className:
        gradients[tournament.tier_level as keyof typeof gradients] ||
        gradients.amateur,
    };
  };
  ...
  return (
    ...
    >
      ...
      {/* Content Section */}
      <CardContent
        className={`p-4 space-y-4 ${isMobile ? 'p-3 space-y-3' : ''}`}
      >
