...
const TournamentManagementFlow: React.FC<TournamentManagementFlowProps> = ({
  ...
  const handleCheckCompletion = async () => {
    ...
    if (isCompleted) {
      toast({
        ...
        description: "All matches have been completed. Results are now available.",
      });
      onShowResults?.();
    }
  };
  ...
  const canGenerateBracket = (tournament.status === 'registration_closed' || tournament.status === 'registration_open') && !hasBracket;
  const canStartTournament = tournament.status === 'registration_closed' && hasBracket;
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex items-center gap-3 p-3 border rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Bracket Status</p>
                  </div>
                </div>
              </div>
            </CardContent>
