...
const StandardCard: React.FC<StandardCardProps> = ({
  ...
  const variants = {
    ...
    tournament: getThemedValue(
      ...
      'bg-gray-800 border-gray-700 text-gray-100 hover:shadow-xl'
    ) + ' rounded-lg border shadow-sm transition-all duration-200',
    challenge: getThemedValue(
      'bg-white border-gray-200 text-gray-900 border-l-blue-500 hover:shadow-md',
      'bg-gray-800 border-gray-700 text-gray-100 border-l-blue-400 hover:shadow-xl'
    ...
    <Card className={cn(variants[variant], className)}>
      {(title || description || headerActions) && (
        <CardHeader className={cn(
          ...
          footer && getThemedValue('border-b border-gray-200', 'border-b border-gray-700')
        )}>
            ...
            <div className="min-w-0 flex-1">
              {title && (
                <CardTitle className={cn(
                  ...
                  'font-semibold',
                  getThemedValue('text-gray-900', 'text-gray-100')
                )}>
                  {title}
                </CardTitle>
              )}
              {description && (
                <p className={cn(
                  ...
                  getThemedValue('text-gray-600', 'text-gray-400'),
                  title && 'mt-1'
                )}>
                  {description}
                </p>
...
const StandardStatusBadge: React.FC<StandardStatusBadgeProps> = ({
  ...
  const getStatusConfig = (): { color: string; text: string } => {
    const configs = {
      tournament: {
        upcoming: { 
          color: getThemedValue('bg-blue-100 text-blue-800', 'bg-blue-900 text-blue-200'), 
          text: 'Sắp diễn ra' 
        },
        registration_open: { 
          color: getThemedValue('bg-green-100 text-green-800', 'bg-green-900 text-green-200'), 
          text: 'Đăng ký' 
        },
        ongoing: { 
          color: getThemedValue('bg-yellow-100 text-yellow-800', 'bg-yellow-900 text-yellow-200'), 
          text: 'Đang diễn ra' 
        },
        completed: { 
          color: getThemedValue('bg-gray-100 text-gray-800', 'bg-gray-700 text-gray-200'), 
          text: 'Hoàn thành' 
        },
        cancelled: { 
          color: getThemedValue('bg-red-100 text-red-800', 'bg-red-900 text-red-200'), 
          text: 'Đã hủy' 
        },
        locked: { 
          color: getThemedValue('bg-purple-100 text-purple-800', 'bg-purple-900 text-purple-200'), 
          text: 'Đã khóa' 
        }
      },
      challenge: {
        ...
        in_progress: { 
          color: getThemedValue('bg-orange-100 text-orange-800', 'bg-orange-900 text-orange-200'), 
          text: 'Đang chơi' 
        },
        completed: { 
          color: getThemedValue('bg-gray-100 text-gray-800', 'bg-gray-700 text-gray-200'), 
        ...
        expired: { 
          color: getThemedValue('bg-gray-100 text-gray-600', 'bg-gray-700 text-gray-400'), 
          text: 'Hết hạn' 
        }
      },
      user: {
  ...
  return (
    <Badge className={cn(config.color, 'text-xs font-medium', className)}>
      {config.text}
    </Badge>
  );
};
    ...
    return (
      <div className={cn('flex flex-col items-center text-center space-y-2', className)}>
        ...
        <div className="space-y-1">
          <div className={cn(sizeConfig.text, 'font-medium text-gray-900 truncate')}>
            {displayName}
          </div>
          <div className="flex flex-col items-center space-y-1">
            ...
            {showPoints && user.spa_points !== undefined && (
              <div className={cn(sizeConfig.subtext, 'text-gray-600')}>
                {user.spa_points} SPA
              </div>
            )}
          </div>
