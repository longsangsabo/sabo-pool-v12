...
export const useAdvancedTournamentActions = () => {
  // Bulk operations
  const bulkUpdateTournamentStatus = useCallback(async (
  ...
  const generateSmartBracket = useCallback(async (
  ...
  const autoScheduleMatches = useCallback(async (
    tournamentId: string,
    preferences: {
      startTime: string;
