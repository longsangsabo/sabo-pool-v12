  ...
  const fetchLeaderboard = useCallback(async (forceRefresh = false) => {
    try {
      ...
      if (mvError || !mvData || mvData.length === 0) {
        ...
        console.warn('Materialized view unavailable, using live data:', mvError);
        ...
        leaderboardData = (liveData || []).map((item: any, index: number) => ({
          id: item.id,
          ...
          win_percentage: item.total_matches > 0 ? 
            Math.round((item.wins / item.total_matches) * 100) : 0,
