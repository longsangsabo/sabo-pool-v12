...
export const useRankRequestModal = (user: User | null | undefined, clubs: {id:string; name:string; address?:string}[]) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await createRankRequest({
        ...
        user_id: user.id,
        evidence_files: [{ id: 'reason', name: 'reason.txt', url: '', size: rankReason.length, type: 'text/plain' }]
      });
  ...
  return {
    ...
    requestedRank, setRequestedRank,
    rankReason, setRankReason,
