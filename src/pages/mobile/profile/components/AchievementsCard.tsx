...
export const AchievementsCard: React.FC<AchievementsCardProps> = ({ theme }) => {
  const achievements = [
    {
      ...
      points: '+100',
      secondary: 'EXP',
      icon: <Trophy className={`w-6 h-6 ${theme === 'dark' ? 'text-violet-300' : 'text-violet-700'}`} />,
      badge: '✨'
    },
    {
      ...
      points: '+80',
      secondary: 'EXP',
      icon: <Zap className={`w-6 h-6 ${theme === 'dark' ? 'text-amber-300' : 'text-amber-600'}`} />
    },
    {
      ...
      points: '+150',
      secondary: 'EXP',
      icon: <Star className={`w-6 h-6 ${theme === 'dark' ? 'text-emerald-300' : 'text-emerald-600'}`} />
    }
  ];
        ...
        {achievements.map((a, i) => (
          <div key={i} className={`group relative overflow-hidden rounded-xl border ${
            ...
            <div className='flex items-center gap-4 p-4'>
              <div className={`relative w-12 h-12 rounded-xl flex items-center justify-center ${a.bg}`}>
                {a.icon}
                {a.badge && (
