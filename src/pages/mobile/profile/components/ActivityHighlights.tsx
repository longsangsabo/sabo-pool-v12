...
export const ActivityHighlights: React.FC<ActivityHighlightsProps> = ({
  ...
  const blockHeader = (icon: React.ReactNode, title: string) => (
    <div className='flex items-center gap-2 mb-4'>
      ...
      <h4
        className={`text-sm font-semibold tracking-wide ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}
      >
        {title}
      </h4>
    </div>
  ...
  return (
    <Card className={`overflow-hidden ${sectionClass}`}>
      <CardHeader className='pb-3'>
        <CardTitle
          className={`text-base font-semibold ${theme === 'dark' ? 'text-slate-100' : 'text-slate-800'}`}
        >
          Tổng quan nhanh
        </CardTitle>
      </CardHeader>
      <CardContent className='space-y-8'>
        ...
        <div>
          ...
          <div className='space-y-3'>
            ...
            >
              <div
                className={`absolute left-0 top-0 bottom-0 w-1 ${theme === 'dark' ? 'bg-emerald-400' : 'bg-emerald-500'}`}
              />
              <div className='flex items-center gap-4 p-4'>
                ...
                >
                  <Trophy
                    className={`w-5 h-5 ${theme === 'dark' ? 'text-emerald-300' : 'text-emerald-600'}`}
                  />
                </div>
                <div className='flex-1 min-w-0'>
                  <div
                    className={`text-sm font-medium mb-1 ${theme === 'dark' ? 'text-slate-100' : 'text-slate-800'}`}
                  >
                    Thắng vs Nguyễn Văn A
                  </div>
                  <div
                    className={`text-xs flex items-center gap-2 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}
                  >
            ...
            >
              <div
                className={`absolute left-0 top-0 bottom-0 w-1 ${theme === 'dark' ? 'bg-rose-400' : 'bg-rose-500'}`}
              />
              <div className='flex items-center gap-4 p-4'>
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                ...
                >
                  <Target
                    className={`w-5 h-5 ${theme === 'dark' ? 'text-rose-300' : 'text-rose-600'}`}
                  />
                </div>
                <div className='flex-1 min-w-0'>
                  <div
                ...
                <div
                  className={`${theme === 'dark' ? 'text-rose-300' : 'text-rose-600'}`}
                >
        ...
        <div>
          ...
          >
            <div className='flex items-center gap-4 p-4'>
              ...
              >
                <Zap
                  className={`w-5 h-5 ${theme === 'dark' ? 'text-blue-300' : 'text-blue-600'}`}
                />
                <div
                  className={`absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full animate-pulse ${theme === 'dark' ? 'bg-amber-400' : 'bg-amber-500'}`}
                />
              </div>
              <div className='flex-1 min-w-0'>
                <div
        ...
        <div>
          ...
          <div className='grid grid-cols-2 gap-3'>
            ...
            >
              ...
              >
                <Target
                  className={`w-4 h-4 ${theme === 'dark' ? 'text-rose-300' : 'text-rose-600'}`}
                />
              </div>
            ...
            >
              ...
              >
                <Trophy
                  className={`w-4 h-4 ${theme === 'dark' ? 'text-indigo-300' : 'text-indigo-600'}`}
                />
              </div>
