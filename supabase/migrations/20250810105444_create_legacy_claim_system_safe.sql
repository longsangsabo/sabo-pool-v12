-- Create Legacy SPA Claim Request System (Safe Version)
...
-- NOTE: Do not drop update_updated_at_column() - it's used by many other tables

-- 3. Drop existing view if exists
DROP VIEW IF EXISTS public.legacy_claim_requests_view;

-- 4. Create legacy_spa_claim_requests table
...
-- 8. Create function to submit legacy SPA claim request
...
-- 9. Create function for SABO admin to approve/reject claim requests
...
BEGIN
  ...
  IF p_action = 'approve' THEN
    -- Add SPA points to user's wallet (only if spa_points_log table exists)
    BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        -- Continue even if spa_points_log doesn't exist
        NULL;
    END;
...
-- 10. Create function to get pending claim requests for SABO admins
...
-- 12. Create helpful view for claim request history
...
-- 13. Add updated_at trigger
-- NOTE: update_updated_at_column() function already exists and is used by many tables
