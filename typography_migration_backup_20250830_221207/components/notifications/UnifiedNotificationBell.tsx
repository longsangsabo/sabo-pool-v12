    ...
    return (
      <div className={`relative ${className}`}>
        <Button 
          variant="ghost" 
          size="sm" 
          className="relative hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-500 dark:text-gray-400"
          disabled
        >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className="text-xs h-6 px-2 text-primary-600 dark:text-blue-400 hover:text-primary-700 dark:hover:text-blue-300"
              ...
              {unreadCount > 0 && (
                <Button
                  ...
                  className="text-xs h-6 px-2 text-neutral-600 dark:text-gray-300 hover:text-neutral-900 dark:hover:text-white"
                >
                  Đánh dấu đã đọc
                </Button>
              )}
              <Button
                ...
                className="h-6 w-6 p-0 text-neutral-500 dark:text-gray-400 hover:text-neutral-700 dark:hover:text-gray-200"
              >
                <X className="h-4 w-4" />
              </Button>
            </div>
        ...
        <div className={`${maxHeight} overflow-y-auto`}>
          {loading ? (
            <div className="p-4 text-center">
              ...
              <p className={`${isCompact ? 'text-xs' : 'text-sm'} text-neutral-500 dark:text-gray-400`}>Đang tải...</p>
            </div>
          ) : notifications.length === 0 ? (
            <div className="p-4 text-center">
              <Bell className={`${isCompact ? 'w-6 h-6' : 'w-8 h-8'} text-gray-400 dark:text-neutral-500 mx-auto mb-2`} />
              <p className={`${isCompact ? 'text-xs' : 'text-sm'} text-neutral-500 dark:text-gray-400`}>Chưa có thông báo nào</p>
            </div>
          ) : (
            <div className="divide-y divide-gray-100 dark:divide-gray-700">
              {notifications.map((notification) => (
                <div
                  key={notification.id}
                  className={`p-4 cursor-pointer hover:bg-neutral-50 dark:hover:bg-gray-700 transition-colors ${
                    !notification.is_read ? 'bg-primary-50/50 dark:bg-blue-900/20' : ''
                  } ${isCompact ? 'p-3' : ''}`}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {!notification.is_read && (
                          <div className={`${isCompact ? 'w-1.5 h-1.5' : 'w-2 h-2'} bg-primary-600 dark:bg-blue-400 rounded-full flex-shrink-0`}></div>
                        )}
                      </div>
                      <p className={`${isCompact ? 'text-xs' : 'text-sm'} text-neutral-600 dark:text-gray-300 mb-2`}>
                        {notification.message}
                      </p>
                      <div className="flex items-center justify-between">
                        <span className={`${isCompact ? 'text-xs' : 'text-xs'} text-gray-400 dark:text-neutral-500`}>
  ...
  return (
    <div className={`relative ${className}`}>
      <Button 
        variant="ghost" 
        size="sm" 
        className="relative hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-700 dark:text-gray-200"
      ...
      >
        ...
        {unreadCount > 0 && (
          <Badge
            variant="destructive"
            className={`absolute -top-1 -right-1 ${variant === 'mobile' ? 'h-4 w-4' : 'h-5 w-5'} rounded-full p-0 flex items-center justify-center text-xs bg-error-500 dark:bg-error-600 text-white`}
          >
