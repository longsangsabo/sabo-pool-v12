...
export const CurrentSeasonCard: React.FC<CurrentSeasonCardProps> = ({
  className,
}) => {
  const {
    loading,
    ...
    try {
      const [season, seasonProgress, seasonComparison] = await Promise.all([
        ...
        getCurrentSeason().then(s =>
          s ? getSeasonProgress(s.season_name, s.season_year) : null
        ),
        getSeasonComparison(),
      ]);
  ...
  const getRankIcon = (rank: number) => {
    ...
    return <span className='text-body-small-medium'>#{rank}</span>;
  };
      ...
      <Card className={`border-yellow-200 bg-warning-50 ${className}`}>
        <CardContent className='p-6 text-center'>
          ...
          <p className='text-warning-800 font-medium'>
            Không có mùa giải đang diễn ra
          </p>
          <p className='text-body-small text-warning-600 mt-1'>
        ...
        <CardContent>
          <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
            <div className='text-center'>
              <p className='text-body-small-neutral'>Mùa giải</p>
              <p className='text-body-large font-bold text-primary-800'>
                {currentSeason.season_name} {currentSeason.season_year}
              </p>
            </div>
            <div className='text-center'>
              <p className='text-body-small-neutral'>Ngày bắt đầu</p>
              <p className='text-body-large font-bold text-primary-800'>
                {formatDate(currentSeason.start_date)}
              </p>
            </div>
            <div className='text-center'>
              <p className='text-body-small-neutral'>Ngày kết thúc</p>
              <p className='text-body-large font-bold text-primary-800'>
                {formatDate(currentSeason.end_date)}
              </p>
            </div>
            <div className='text-center'>
              <p className='text-body-small-neutral'>Còn lại</p>
              <p className='text-body-large font-bold text-primary-800'>
          ...
          {progress && (
            <div className='mt-6'>
              ...
              <div className='flex justify-between text-caption-neutral mt-1'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
              ...
              <div>
                <h4 className='font-medium text-neutral-900 mb-3'>
                  {comparison.current_season.season_name}{' '}
                  {comparison.current_season.season_year}
                </h4>
                <div className='space-y-2'>
                  <div className='flex justify-between'>
                    ...
                    <span className='font-medium'>
                      {comparison.current_season.total_players}
                    </span>
                  </div>
                  <div className='flex justify-between'>
                    <span className='text-body-small-neutral'>
                    ...
                    <span className='font-medium'>
                      {comparison.current_season.highest_points.toLocaleString(
                        'vi-VN'
                      )}
                    </span>
                  </div>
                  <div className='flex justify-between'>
                    ...
                    <span className='font-medium'>
                      {Math.round(comparison.current_season.average_points)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h4 className='font-medium text-neutral-900 mb-3'>
                  {comparison.previous_season.season_name}{' '}
                  {comparison.previous_season.season_year}
                </h4>
                <div className='space-y-2'>
                  <div className='flex justify-between'>
                    <span className='text-body-small-neutral'>
                    ...
                    <span className='font-medium'>
                      {comparison.previous_season.total_players}
                    </span>
                  </div>
                  <div className='flex justify-between'>
                    <span className='text-body-small-neutral'>
                    ...
                    <span className='font-medium'>
                      {comparison.previous_season.highest_points.toLocaleString(
                        'vi-VN'
                      )}
                    </span>
                  </div>
                  <div className='flex justify-between'>
                    ...
                    <span className='font-medium'>
                      {Math.round(comparison.previous_season.average_points)}
                    </span>
                  </div>
                </div>
              </div>
                    ...
                    .map((player, index) => (
                      ...
                      >
                        ...
                        <div className='flex items-center space-x-4'>
                          <div className='text-center'>
                            <span className='text-caption-neutral'>Hạng</span>
                          ...
                          <div className='text-center'>
                            <span className='text-caption-neutral'>Điểm</span>
                            <div className='flex items-center space-x-1'>
                              <span className='text-sm'>
      ...
      {error && (
        <Card className='border-error-200 bg-error-50'>
          <CardContent className='p-4'>
            <p className='text-error-600'>Lỗi: {error}</p>
          </CardContent>
        </Card>
      )}
    </div>
