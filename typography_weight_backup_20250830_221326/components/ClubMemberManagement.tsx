...
const ClubMemberCard = ({ member, onViewDetails }: ClubMemberCardProps) => {
  ...
  const getStatusBadge = () => {
    if (member.verification_status === 'verified') {
      return (
        <span className='inline-flex items-center px-2.5 py-0.5 rounded-full text-caption font-medium bg-success-100 text-success-800'>
          ✓ Đã xác thực
        </span>
      );
    }
    return (
      <span className='inline-flex items-center px-2.5 py-0.5 rounded-full text-caption font-medium bg-neutral-100 text-neutral-800'>
        Chưa xác thực
      </span>
    );
  };
      ...
      <div className='flex items-center justify-between'>
        <div className='flex items-center space-x-3'>
          ...
          <div className='flex-1 min-w-0'>
            <h4 className='text-body-small font-medium text-neutral-900 truncate'>
              {member.display_name || member.username}
            </h4>
            <p className='text-body-small text-neutral-500'>
              {member.phone || 'Không có số điện thoại'}
            </p>
            <p className='text-body-small font-medium text-primary-600'>
              {getRankDisplay()}
            </p>
            <div className='flex items-center gap-2 mt-1'>
              <span className='text-caption font-medium text-info-600'>
      ...
      <Card>
        <CardContent className='pt-6'>
          <div className='flex items-center justify-center py-8'>
            ...
            <span className='ml-2 text-neutral-600'>Đang tải thành viên...</span>
          </div>
        </CardContent>
      </Card>
    );
