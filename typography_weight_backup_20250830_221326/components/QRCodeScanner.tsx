...
export const QRCodeScanner: React.FC<QRCodeScannerProps> = ({
  ...
  if (error) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className='text-error-600'>Lỗi</CardTitle>
        </CardHeader>
        <CardContent>
          <p className='text-body-small text-neutral-600'>{error}</p>
      ...
      {showMatchForm && scannedTable && (
        <Card>
          ...
          <CardContent>
            <div className='space-y-4'>
              <div>
                <label className='block text-body-small font-medium mb-2'>
                  Loại game
                </label>
                <select
                  value={gameType}
