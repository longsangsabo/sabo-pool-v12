  ...
  const renderNavigationItem = (item: any, index: number) => {
    ...
    return (
      <NavLink
        ...
        to={item.href}
        className={cn(
          'flex items-center gap-3 px-3 py-2 rounded-lg text-body-small font-medium transition-all duration-200',
          'hover:bg-accent hover:text-accent-foreground',
          'focus:bg-accent focus:text-accent-foreground focus:outline-none',
          isActive ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground',
          collapsed && 'justify-center px-2'
        )}
        className="transition-dynamic" style={{ "--transition-property": "all", "--transition-duration": PLAYER_SIDEBAR_TOKENS.animation.transition }}
      >
        ...
        {!collapsed && (
          <div className='flex-1 min-w-0'>
            ...
            {item.description && (
              <p className='text-caption-muted truncate mt-0.5'>
                {item.description}
              </p>
            )}
          </div>
      ...
      <div className='p-4 border-t border-border'>
        {!collapsed && (
          <div className='space-y-3'>
            <div className='flex items-center justify-between'>
              <span className='text-caption-muted'>Theme</span>
              <ThemeToggle />
            </div>
