...
export const MilestoneCard: React.FC<MilestoneCardProps> = ({ data }) => {
  ...
  return (
    <Card className={cn('relative overflow-hidden group', data.completed && 'ring-1 ring-green-500/40')}>      
      {data.completed && (
        <div className='absolute top-2 right-2 text-success-600 flex items-center gap-1 text-caption font-medium'>
      ...
      <CardContent className='p-4 space-y-3'>
        <div className='flex items-start justify-between gap-3'>
          <div>
            ...
            <p className='text-caption-muted line-clamp-2'>{data.description}</p>
          </div>
