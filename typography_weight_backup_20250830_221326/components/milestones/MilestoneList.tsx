...
export const MilestoneList: React.FC<Props> = ({ milestones, category, loading }) => {
  if (loading) return <p className='text-body-small-muted'>Loading milestones...</p>;
  const filtered = milestones.filter(m => m.category === category);
  if (!filtered.length) return <p className='text-body-small-muted'>No milestones in this category.</p>;
  return (
