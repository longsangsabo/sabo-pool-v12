...
interface Match {
  ...
  assigned_table_id: string | undefined;
  player1?: {
    full_name: string;
    display_name?: string;
  ...
  score_edited_by?: string | undefined;
  score_edit_count?: number;
  last_score_edit?: string | undefined;
}
...
export const EnhancedMatchCard: React.FC<EnhancedMatchCardProps> = ({
  ...
  const getMatchStatusBadge = () => {
    switch (match.status) {
      case 'completed':
        return (
          <Badge className='bg-success-500 hover:bg-success-600'>Hoàn thành</Badge>
        );
      case 'in_progress':
      case 'ongoing':
        return (
      ...
      case 'ready':
        return (
          <Badge className='bg-warning-500 hover:bg-orange-600'>Sẵn sàng</Badge>
        );
      case 'scheduled':
  ...
  const isPlayerWinner = (playerId: string | undefined) => {
    return match.winner_id === playerId;
  };

  const getPlayerCardStyle = (playerId: string | undefined) => {
    if (match.status === 'completed' && isPlayerWinner(playerId)) {
      return 'bg-success-50 border border-success-200 text-success-800 font-semibold';
    }
    ...
    >
      <CardContent className='p-5'>
        ...
        <div className='flex items-center justify-between mb-4'>
          <Badge
            variant='outline'
            className='px-3 py-1 text-body-small font-bold bg-primary/10 border-primary/30 text-primary'
          >
        ...
        <div className='space-y-3'>
          ...
          <div
            className={`p-3 rounded-xl border-l-4 transition-all duration-200 ${
              match.player1
                ? 'bg-primary-50/50 border-blue-400 hover:bg-primary-50'
          ...
          >
                ...
                {match.player1 ? (
                  <div className='flex items-center gap-3'>
                    ...
                    <div className='flex-1 min-w-0'>
                      <div className='font-semibold text-foreground text-body-small truncate flex items-center gap-2'>
                        {match.player1.display_name || match.player1.full_name}
                        <span className='text-caption bg-primary text-primary-foreground px-2 py-1 rounded-full font-bold'>
                  ...
                  <div className='flex items-center gap-3'>
                    ...
                    <span className='text-body-small-muted font-medium'>
                      ⏳ Chờ kết quả
                    </span>
                  </div>
                )}
            ...
            {isClubOwner && match.player1_id && match.player2_id && (
              <div className='mt-2 p-3 bg-primary-50 border border-primary-200 rounded-lg'>
                <div className='flex items-center gap-3'>
                  <span className='text-body-small font-medium text-primary-700 min-w-[60px]'>
                    Tỷ số:
                  </span>
                  <input
                    ...
                    tabIndex={0}
                  />
                  <span className='text-caption text-primary-600 font-medium'>P1</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className='flex items-center justify-center py-2'>
            <div className='bg-gradient-to-r from-primary to-primary/80 text-primary-foreground px-4 py-1 rounded-full text-body-small font-bold shadow-sm'>
              VS
            </div>
          </div>
          ...
          <div
            className={`p-3 rounded-xl border-l-4 transition-all duration-200 ${
              match.player2
                ? 'bg-info-50/50 border-purple-400 hover:bg-info-50'
                : 'bg-muted/30 border-muted-foreground'
          ...
          >
            ...
            {isClubOwner && match.player1_id && match.player2_id && (
              <div className='mt-2 p-3 bg-info-50 border border-purple-200 rounded-lg'>
                <div className='flex items-center gap-3'>
                  <span className='text-body-small font-medium text-info-700 min-w-[60px]'>
                    Tỷ số:
                  </span>
                  <input
                    ...
                    tabIndex={0}
                  />
                  <span className='text-caption text-info-600 font-medium'>
                    P2
                  </span>
                </div>
              </div>
        ...
        <div className='text-center mt-4'>
          <Badge
            ...
            className='text-body-small px-4 py-1.5 font-medium shadow-sm'
          >
            {match.status === 'completed'
              ? '🏆 Hoàn thành'
              : match.status === 'ongoing'
        ...
        {isClubOwner && match.player1_id && match.player2_id && (
          <div className='mt-4 space-y-3'>
            ...
            {match.score_edit_count && match.score_edit_count > 0 && (
              <div className='text-caption-muted flex items-center gap-1 justify-center'>
