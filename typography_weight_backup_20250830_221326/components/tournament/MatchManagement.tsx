...
export const MatchManagement: React.FC<MatchManagementProps> = ({
  ...
  const getMatchStatus = (match: Match) => {
    if (match.status === 'completed') {
      return <Badge className='bg-success-600'>Hoàn thành</Badge>;
    } else if (match.status === 'ongoing') {
      return <Badge className='bg-primary-600'>Đang diễn ra</Badge>;
                  ...
                  <Card key={match.id} className='border-l-4 border-l-primary'>
                    <CardContent className='pt-4'>
                      ...
                      {match.scheduled_time && (
                        <div className='mt-3 flex items-center gap-2 text-body-small-muted'>
