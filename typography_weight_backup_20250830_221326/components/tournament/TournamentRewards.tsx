...
const PositionIcon = ({ position }: { position: number }) => {
  ...
  return iconMap[position] || <Gift className='w-5 h-5 text-neutral-500' />;
};
    ...
    <div className='grid grid-cols-4 gap-3 p-3 rounded-lg bg-muted/50 hover:bg-muted transition-colors text-sm'>
      ...
      <div className='flex flex-wrap gap-1'>
        {showElo && (
          <Badge
            variant='secondary'
            className='bg-primary-100 text-primary-700 dark:bg-blue-900 dark:text-blue-300 text-xs'
          >
            +{eloReward} ELO
          </Badge>
        )}
        {showSpa && (
          <Badge
            variant='secondary'
            className='bg-warning-100 text-warning-700 dark:bg-yellow-900 dark:text-yellow-300 text-xs'
          >
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <Card>
        ...
        <CardContent className='space-y-3'>
          {/* Table Header */}
          <div className='grid grid-cols-4 gap-3 p-2 bg-muted rounded-lg text-body-small font-medium text-muted-foreground'>
            <div>Vị trí</div>
            <div>Điểm</div>
            <div>Tiền thưởng</div>
            <div>Hiện vật</div>
