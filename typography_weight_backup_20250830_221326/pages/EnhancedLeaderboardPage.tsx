...
const EnhancedLeaderboardPage = () => {
  ...
  const getRankColor = (rank: string) => {
    ...
    if (rank.startsWith('K')) return 'border-neutral-200 bg-neutral-50 text-neutral-800';
  ...
  return (
    <div className='min-h-screen bg-neutral-50'>
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
        {/* Header */}
        <div className='mb-8'>
          <h1 className='text-3xl font-bold text-neutral-900'>Bảng xếp hạng</h1>
                ...
                <Card key={region}>
                  <CardContent className='p-6'>
                    ...
                    <Button
                      ...
                      className='mt-4 w-full text-primary-600 hover:text-primary-800 text-body-small font-medium'
                    >
                      Xem bảng xếp hạng{' '}
                      {regionNames[region as keyof typeof regionNames]}
                    </Button>
        ...
        <Card>
          {loading ? (
            <CardContent className='text-center py-12'>
              ...
              <p className='mt-2 text-neutral-600'>Đang tải bảng xếp hạng...</p>
            </CardContent>
          ) : rankings.length === 0 ? (
            <CardContent className='text-center py-12'>
                ...
                <tbody className='bg-white divide-y divide-gray-200'>
                  {rankings.map((ranking, index) => (
                    <tr key={ranking.id} className='hover:bg-neutral-50'>
                          ...
                          {index < 3 ? (
                            ...
                            >
                              {index === 0 ? (
                                ...
                                <Medal className='w-5 h-5 text-neutral-600' />
                              ) : (
                                <Award className='w-5 h-5 text-warning-600' />
                              )}
                            </div>
                          ) : (
                            <span className='text-body-large font-bold text-neutral-600'>
                              #{index + 1}
                            </span>
                          )}
                        </div>
                      ...
                      <td className='px-6 py-4 whitespace-nowrap'>
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-body-small font-medium border ${getRankColor(ranking.current_rank)}`}
                        >
                          {ranking.current_rank}
                        </span>
                      </td>

                      <td className='px-6 py-4 whitespace-nowrap text-body-small font-medium text-neutral-900'>
                        {ranking.current_points.toLocaleString('vi-VN')}
                      </td>

                      <td className='px-6 py-4 whitespace-nowrap text-body-small font-medium'>
