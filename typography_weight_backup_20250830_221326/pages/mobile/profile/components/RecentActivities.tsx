  ...
  >
    <div className='flex items-center gap-4 p-3'>
      ...
      >
        ...
        <div
          className={`absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full ${
            theme === 'dark' ? 'bg-blue-400' : 'bg-primary-500'
          }`}
        ></div>
      </div>
      <div className='flex-1 min-w-0'>
        <div
          className={`text-body-small font-medium mb-0.5 ${
      ...
      {points && (
        ...
        >
          <div className='text-body-small font-bold'>{points.value}</div>
          <div className='text-caption font-medium opacity-80'>{points.label}</div>
        </div>
      )}
    </div>
  </div>
...
export const RecentActivities: React.FC<RecentActivitiesProps> = ({
  ...
  const getActivityIcon = (iconType: string) => {
    switch (iconType) {
      case 'trophy':
        return <Trophy className={`w-4 h-4 ${theme === 'dark' ? 'text-blue-300' : 'text-primary-600'}`} />;
      case 'star':
      ...
      case 'award':
        return <Award className={`w-4 h-4 ${theme === 'dark' ? 'text-purple-300' : 'text-info-600'}`} />;
      default:
        return <Activity className={`w-4 h-4 ${theme === 'dark' ? 'text-gray-300' : 'text-neutral-600'}`} />;
    }
  };
  ...
  const fallbackActivities = [
    {
      icon: (
        <Activity
          className={`w-4 h-4 ${theme === 'dark' ? 'text-gray-300' : 'text-neutral-600'}`}
        />
      ),
      ...
      {loading ? (
        <div className="flex justify-center py-8">
          <div className="text-center">
            ...
            <p className={theme === 'dark' ? 'text-gray-300' : 'text-neutral-600'}>
              Đang tải hoạt động...
            </p>
          </div>
        </div>
        ...
        <CardHeader className='pb-4 border-b border-slate-200/10'>
          <CardTitle
            className={`text-body font-semibold flex items-center justify-between ${
              theme === 'dark' ? 'text-slate-100' : 'text-slate-800'
            }`}
          >
