...
export const SABO32DoubleEliminationViewer: React.FC<SABO32ViewerProps> = ({
  ...
  return (
    <div className="sabo32-tournament-container space-y-6">
      ...
      {/* Header */}
      <Card className="bg-gradient-to-r from-blue-50 to-purple-50 border-primary-200">
        <CardHeader>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center gap-3 p-3 bg-primary-100 rounded-lg">
              ...
              <div>
                <div className="font-semibold text-primary-800">Group A</div>
                <div className="text-body-small text-primary-600">{groupAProgress}% Complete</div>
                {isGroupACompleted && (
                  <Badge className="mt-1 bg-success-100 text-success-800">✅ Qualified</Badge>
                )}
              </div>
            </div>
            
            {/* Group B Status */}
            <div className="flex items-center gap-3 p-3 bg-info-100 rounded-lg">
              <div className="flex-shrink-0">
                <Crown className="h-5 w-5 text-info-600" />
              </div>
              <div>
                <div className="font-semibold text-purple-800">Group B</div>
                <div className="text-body-small text-info-600">{groupBProgress}% Complete</div>
                {isGroupBCompleted && (
                  <Badge className="mt-1 bg-success-100 text-success-800">✅ Qualified</Badge>
                )}
              </div>
            ...
            <div className="flex items-center gap-3 p-3 bg-warning-100 rounded-lg">
              <div className="flex-shrink-0">
                <Target className="h-5 w-5 text-warning-600" />
              </div>
              <div>
                <div className="font-semibold text-warning-800">Cross-Bracket</div>
                <div className="text-body-small text-warning-600">
                  {canStartCrossBracket ? 'Ready' : 'Waiting for groups'}
                </div>
                {canStartCrossBracket && (
                  <Badge className="mt-1 bg-warning-200 text-warning-800">🎯 Active</Badge>
                )}
              </div>
            </div>
          </div>
...
const GroupBracketViewer: React.FC<GroupBracketViewerProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Group Header */}
      <Card className={`${groupId === 'A' ? 'bg-primary-50 border-primary-200' : 'bg-info-50 border-purple-200'}`}>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <span className="flex items-center gap-2">
              <Crown className={`h-5 w-5 ${groupId === 'A' ? 'text-primary-600' : 'text-info-600'}`} />
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-success-700">🏆 Winners Bracket</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {Object.entries(winnersRounds).map(([round, matches]) => (
              <div key={round}>
                <h4 className="font-semibold mb-2 text-success-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-warning-700">💔 Losers Branch A</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {Object.entries(losersARounds).map(([round, matches]) => (
                <div key={round}>
                  <h4 className="font-semibold mb-2 text-warning-700">
                    Round {round} ({matches.length} matches)
                  </h4>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-error-700">💔 Losers Branch B</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {Object.entries(losersBRounds).map(([round, matches]) => (
                <div key={round}>
                  <h4 className="font-semibold mb-2 text-error-700">
                    Round {round} ({matches.length} matches)
                  </h4>
                  <div className="grid grid-cols-1 gap-3">
                    {matches.map(match => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {groupData.final.map((match, index) => (
                <div key={match.id} className="max-w-md mx-auto">
                  <h4 className="text-body-small font-medium text-warning-700 mb-2 text-center">
                    Final {index + 1}
                  </h4>
                  <SABOMatchCard
                    match={match}
...
const CrossBracketViewer: React.FC<CrossBracketViewerProps> = ({
    ...
    return (
      <Card className="bg-neutral-50 border-neutral-200">
        <CardContent className="text-center py-12">
          ...
          <h3 className="text-body-large font-semibold text-neutral-600 mb-2">
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-primary-700">🎯 Semifinals</CardTitle>
      ...
      {crossBracketData.final.length > 0 && (
        <Card className="bg-gradient-to-r from-yellow-100 to-orange-100 border-yellow-300">
          <CardHeader>
            <CardTitle className="text-center text-warning-800">
              🏆 CHAMPIONSHIP FINAL
            </CardTitle>
